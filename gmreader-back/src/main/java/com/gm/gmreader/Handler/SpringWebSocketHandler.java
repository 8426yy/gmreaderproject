...
import com.gm.gmreader.dao.ChatMapper;
import com.gm.gmreader.dao.ChatvoMapper;
...
public class SpringWebSocketHandler extends TextWebSocketHandler {
    ...
    public Chatvo getChatvo(Integer from,Integer to,String content)
    {
        ChatvoExample chatvoExample =new ChatvoExample();
        ChatvoExample.Criteria criteria = chatvoExample.createCriteria();
        criteria.andFromidEqualTo(from);
        criteria.andToidEqualTo(to);
        criteria.andContentEqualTo(content);
        chatvoExample.setOrderByClause("chid DESC");
    ...
    public void sendToUser(Integer from,Integer to ,String content)
    {
        Date date = new Date();
        Chat chat = new Chat();
        chat.setFromid(from);
        ...
        Chatvo chatvo=getChatvo(from,to,content);
        if(chatvo != null)
        {
            JsonResult res = ResultTool.success();
            res.setData(chatvo);
            TextMessage textMessage=new TextMessage(JSON.toJSONString(res));
            sendMessageToUser(to.toString(),textMessage);
        }
    }
